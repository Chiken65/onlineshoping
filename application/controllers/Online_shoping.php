...
class Online_shoping extends CI_Controller
{
	...
	{
		$data['home_banners'] = ['head1.png', 'head2.jpg', 'head3.jpg'];
	...
	public function login_check()
	{
		...
		print_r($login_check[0]);
		echo $login_check[0]->type;
		...
		if (sizeof($login_check) == 0) {
			...
			if ($login_check[0]->type == "user") {
				echo "<script>
				localStorage.setItem('type', 'user');
				localStorage.setItem('email', '" . $login_check[0]->email . "');
				localStorage.setItem('name', '" . $login_check[0]->name . "');
				localStorage.setItem('phone_no', '" . $login_check[0]->phone_no . "');
				localStorage.setItem('gender', '" . $login_check[0]->gender . "');
				localStorage.setItem('address', '" . $login_check[0]->address . "');
				</script>";
				$this->load->view('common/header');
				$this->load->view('common/userNavigation');
	...
	public function registerUserInfo()
	{
		...
		if ($check) {
			$this->session->set_flashdata('msg1', '<div class="alert alert-success text-center">Added Successfully</div>');
	...
	{
		$data['availableItems'] = $this->sitedata->availableItems();

		$this->load->view('common/header');
		$this->load->view('common/navigation');
		$this->load->view('availableItems', $data);
	}
